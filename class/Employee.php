...
class Employee extends Database implements iEmployee {
	...
	public function insert_employee($fN, $mN, $lN, $pos, $rm, $type)
	{
		$un = $fN.'_'.$lN;
		...
		$sql = "INSERT INTO tbl_employee
				(emp_fname, emp_mname, emp_lname, pos_id, rm_id, type_id, emp_un, emp_pass)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?);
				";
		return $this->insertRow($sql, [$fN, $mN, $lN, $pos, $rm, $type, $un, $pass]);
	}

	public function update_employee($fN, $mN, $lN, $pos, $rm, $type, $eid)
	{
		$sql = "UPDATE tbl_employee
				SET emp_fname = ?, emp_mname = ?, emp_lname = ?, pos_id = ?, rm_id = ?, type_id = ?
				WHERE emp_id = ?;
		";
		return $this->updateRow($sql, [$fN, $mN, $lN, $pos, $rm, $type, $eid]);
	}
	...
	public function employee_rooms()
	{
	...
	public function insert_employee_room($room)
	{
		$sql="INSERT INTO tbl_rm(rm_desc)
			  VALUES(?);
			";
		return $this->insertRow($sql, [$room]);
	}
