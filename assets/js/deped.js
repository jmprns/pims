...
function item_profile(iID)
{
	...
	$.ajax({
		...
		success: function(event){
			...
			$('.item-property').val(event.item_ptyno);
			$('.item-model').val(event.item_modno);
			$('.item-quantity').val(Number(event.item_quantity).toLocaleString('en'));
			$('.item-recieved').val(event.item_rcvdate);
...
function fill_update_modal(iID){
	$.ajax({
			...
			success: function (data) {
				...
				$('#propertyNumber-update').val(data.item_ptyno);
				$('#modelNumber-update').val(data.item_modno);
				$('#brand-update').val(data.item_brand);
				$('#quantity-update').val(data.item_quantity);
				$('#rcvDate-update').val(data.item_rcvdate);
...
$(document).on('submit', '#add-room-form', function(event) {
	event.preventDefault();
	/* Act on the event */
	var room = $('input[id=room]');
	if(room.val() == ''){
		room.parent().parent().addClass('has-error');
	}else{
		room.parent().parent().removeClass('has-error');
		$.ajax({
				url: '../data/add_room.php',
				type: 'post',
				dataType: 'json',
				data: {
					off : room.val()
				...
				error: function(){
					alert('Error: L366+ on submit #add-room-form');
				}
			});
	}
});
/*end add new room logic here*/
...
$('#confirm-room').click(function(event) {
	/* Act on the event */
	$.ajax({
			url: '../data/delete_room.php',
			type: 'post',
			dataType: 'json',
			data: {
				rm_id : rm_id
...
//end update position
$(document).on('submit', '#upt-room-form', function(event) {
	event.preventDefault();
	/* Act on the event */
	var validate = "";
	var form_att = new Array(
...
var rm_id;
function fill_room_form(id,room){
	rm_id = id;
	$('#off-id').val(rm_id)
	$('#upt-room').val(room)
...
// $(document).on('submit', '#modal-update-room', function(event) {
// 	event.preventDefault();
// 	/* Act on the event */
// 	alert('bogkot');
...
$(document).on('change', '#catID', function(event) {
	event.preventDefault();
	/* Act on the event */
	// $('#propertyNumber').attr('type', 'hidden');
	var cat = $('#catID').val();
	if(cat == 2){
		$('#sr').hide();
		$('#mn').hide();
